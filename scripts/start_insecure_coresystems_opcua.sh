...
nohup java -jar $(find . -maxdepth 1 -name arrowhead-serviceregistry-sql-\*.jar | sort | tail -n1) -d -daemon -opcua &> insecure_sr.log &
...
nohup java -jar $(find . -maxdepth 1 -name arrowhead-authorization-\*.jar | sort | tail -n1) -d -daemon -opcua &> insecure_auth.log &
...
nohup java -jar $(find . -maxdepth 1 -name arrowhead-eventhandler-\*.jar | sort | tail -n1) -d -daemon -opcua &> insecure_eventhandler.log &
...
nohup java -jar $(find . -maxdepth 1 -name arrowhead-orchestrator-\*.jar | sort | tail -n1) -d -daemon -opcua &> insecure_orch.log &
